...
export default function HomePage() {
    ...
    const handleSendEmail = useCallback(async () => {
        ...
        const { data, error } = await sendEmail(emailAddress.trim(), subject.trim(), pendingHTML);
    ...
    return (
        <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
            ...
            {/* Email Dialog */}
            {showDialog && (
                <DialogComponent isSending={isSending} showDialog={showDialog} setShowDialog={setShowDialog} emailAddress={emailAddress} setEmailAddress={setEmailAddress} subject={subject} setSubject={setSubject} pendingHTML={pendingHTML} setPendingHTML={setPendingHTML} handleSendEmail={handleSendEmail} />
            )}
        </div>
    );
}
